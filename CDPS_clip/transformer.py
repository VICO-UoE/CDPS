...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(
        ...
        self.record_weight = attn_weights
        return attn_output
        ...
        self.mlp = nn.Sequential(
            OrderedDict(
                [
                    ...
                    (
                        "ln",
                        norm_layer(mlp_width) if scale_fc else nn.Identity(),
                    ),  # from NormFormer / Foundation Transformers
                    ("c_proj", nn.Linear(mlp_width, d_model)),
                ]
            )
        )
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.grad_checkpointing = False
        self.feats_each_layer = []
        self.attn_each_layer = []
...
class TextTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.correct_cls_mask = (
            correct_cls_mask  # use the correct cls mask for CoCa (original is wrong)
        )
...
def lock_text_tower(
    ...
    text_params["positional_embedding"] = getattr(
        text_module, "positional_embedding", None
    )
